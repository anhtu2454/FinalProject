...
namespace Soor.RopeGenerator
{
    public class Rope
    {
        ...
        /// <summary>
        /// Giới hạn cứng số đốt dây tối đa -- quá nhiều đốt (nhiều CharacterJoint nối tiếp)
        /// khiến PhysX mô phỏng không ổn định, gây giật/rung khi chạy.
        /// </summary>
        private const int MaxSegmentCount = 15;
        ...
        public void SpawnRope()
        {
            ...
            if (ropeSegmentCount > MaxSegmentCount)
            {
                ropeSegmentCount = MaxSegmentCount;
                // tính lại khoảng cách giữa các đốt để tổng chiều dài dây vẫn đúng bằng Rope Length đã đặt
                _ropeData.segmentsDistance = _ropeData.ropeLength / ropeSegmentCount;
            }
        ...
        {
            // Y-scale theo segmentsDistance (không phải hằng số cố định theo thickness) --
            // để khi số đốt bị giới hạn (dãn khoảng cách ra), từng đốt tự giãn theo, tránh bị hở.
            ropeSegment.transform.localScale = new Vector3(_ropeData.ropeThickness, _ropeData.segmentsDistance / 2f, _ropeData.ropeThickness);
